...
3. **Scale/concurrency** — Single process or distributed? Multiple threads/users hitting it at once? (You can say "I'll design for single-process, multi-threaded, and call out where I'd change things for distributed" — that's a senior move.)
...
|---|---|---|---|
| **Inheritance ("is-a")** | Subtype specializes supertype | `class Car extends Vehicle` / `class Car(Vehicle)` | `Car is-a Vehicle` |
| **Interface implementation** | Contract, no shared state | `class Car implements Parkable` | `Car implements Payable` |
| **Composition ("has-a", owns, can't outlive")** | Strong ownership; part dies with whole | field + created/destroyed together | `Car has-a Engine` (engine has no meaning outside this car) |
| **Aggregation ("has-a", shared/independent lifetime)** | Weak ownership; part outlives whole | field, but reference to externally-owned object | `ParkingLot has-a List<ParkingSpot>` but spots could conceptually be reused/queried elsewhere |
...
| **Multiplicity** | 1-1, 1-many, many-many | collection type in the field | `ParkingLot 1 -- * ParkingSpot` |
...
`[*]` = one-to-many, `[1]` = exactly one, `(interface)` flags a Strategy/polymorphic seam.
...
2. **Underline every verb** → candidate methods (`park()`, `unpark()`, `calculateFee()`, `pay()`).
...
4. **Look for "varies by type" language** ("fee differs by vehicle type," "different payment methods," "different notification channels") — every one of these phrases is a [Strategy pattern](patterns/03-behavioral-patterns.md#strategy) seam, and should become an interface, not an `if/else` on a type field.
